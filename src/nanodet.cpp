...
int NanoDet::resize_uniform(const cv::Mat& src, cv::Mat& dst, cv::Size dst_size, object_rect& effect_area)
{
    int w = src.cols;
    int h = src.rows;
...
void NanoDet::draw_bboxes(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes, object_rect effect_roi)
{
    static const char* class_names[] = { "meter" };
...
void NanoDet::convert_boxes(const std::vector<BoxInfo>& bboxes, const object_rect& effect_roi, int src_w, int src_h, std::vector<Object>& objects) {
    int dst_w = effect_roi.width;
    int dst_h = effect_roi.height;
    float width_ratio = static_cast<float>(src_w) / dst_w;
    float height_ratio = static_cast<float>(src_h) / dst_h;
    ...
    for (const auto& bbox : bboxes) {
        ...
        Object obj;
        obj.rect = cv::Rect_<float>(cv::Point_<float>(x1, y1), cv::Point_<float>(x2, y2));
