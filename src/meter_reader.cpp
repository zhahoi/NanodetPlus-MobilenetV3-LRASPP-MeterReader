...
float MeterReader::reader_process(cv::Mat& img, cv::Mat& mask) {
    ...
    cv::morphologyEx(scale_mask, scale_mask, cv::MORPH_OPEN, kernel);
    // 对指针也进行开运算
    cv::morphologyEx(pointer_mask, pointer_mask, cv::MORPH_OPEN, kernel_);

#ifdef IMG_TEST
    cv::imshow("scale_mask2", scale_mask);
    cv::waitKey(0);
#endif 
    ...
    getCenterLocation(scale_mask, center_location);
    ...
    // 根据量程和角度计算最后的读数
    float scale_value = std::abs(getScaleValue(angle_ratio));
...
std::vector<float> MeterReader::multi_read_process(const std::vector<cv::Mat>& input_image, const std::vector<cv::Mat>& output)
{
    ...
    for (int i_num = 0; i_num < meter_num; i_num++)
    {
        ...
        float scale_value = std::abs(reader_process(image_, mask_));
