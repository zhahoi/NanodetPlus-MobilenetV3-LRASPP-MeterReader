...
void MeterReaderV2::creat_line_image(std::vector<unsigned char>& seg_image, std::vector<unsigned char>& output)
{
	...
	for (int row = 0; row < LINE_HEIGH; row++)
	{
		for (int col = 0; col < LINE_WIDTH; col++)
		{
			...
			image_x = int(circle_center[0] + rho * cos(theta) + 0.5f);
			image_y = int(circle_center[1] - rho * sin(theta) + 0.5f);
...
void MeterReaderV2::read_process_one(const cv::Mat& seg_image, READ_RESULT& read_result)
{
    // 将单张图像转换为向量
    std::vector<unsigned char> seg_image_vector = mat2vector(seg_image * 100);
    std::vector<unsigned char> line_result;
    creat_line_image(seg_image_vector, line_result);
    ...
    read_result = get_meter_reader(scale_mean_data, pointer_data);
}

float MeterReaderV2::get_result_one(const READ_RESULT& read_result)
{
    float result = 0.0;
    std::string unit_str;

    if (read_result.scale_num > TYPE_THRESHOLD_HIGH)
    {
        result = read_result.scales * meter_config[0].scale_value;
        unit_str = meter_config[0].str;
    }
    else if (read_result.scale_num > TYPE_THRESHOLD_LOW)
    {
        result = (read_result.scales - 1) * meter_config[1].scale_value;
        unit_str = meter_config[1].str;
    }
    else
    {
        result = (read_result.scales - 1) * meter_config[2].scale_value;
        unit_str = meter_config[2].str;
    }

    printf("scale_num:%d, scales:%f, ratio:%f, unit:%f.\n", read_result.scale_num, read_result.scales, read_result.ratio, meter_config[0].scale_value);
